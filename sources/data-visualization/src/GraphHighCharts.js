...
export const GraphHighCharts = ({ graphLightMode, selectedIndicatorsList }) => {
  ...
  const updateGraphIndicators = () => {
        ...
        if (indicator === 'volume') {
          if (!chart.get('volumeAxis')) {
            chart.addAxis({
              ...
              offset: 0,
              lineWidth: 1
            }, false, false); // second false to delay redraw
          }
    ...
    const axesToRemove = chart.yAxis.filter(axis => {
      return axis.userOptions.id !== 'mainAxis' && axis.userOptions.id !== 'navigator-y-axis' && !selectedIndicatorsList.includes(axis.userOptions.id.replace('Axis', ''));
    });
    ...
    chart.yAxis.forEach((axis) => {
      ...
      if (axis.options.id !== 'mainAxis' && axis.options.id !== 'navigator-y-axis') {
        const indicatorIndex = selectedIndicatorsList.indexOf(axis.options.id.replace('Axis', ''));
        if (indicatorIndex !== -1) { // If the axis is found in the selectedIndicatorsList
