...
export const Graph = () => {
    ...
    useEffect(() => {
        const currentIndicatorNames = new Set(indicatorsTransform.map(indicator => indicator.name));
    ...
    useCreateGraph(mainGraphRef, indicatorGraphsRefs, graphDataSeries, indicatorsTransform, graphSeriesType, graphLightMode);
...
/*     const updateChart = () => {
    ...
    const changeData = () => {
        setGraphDataSeries([
            { time: '2018-12-22', open: 80.16, high: 82.84, low: 36.16, close: 45.72 },
            { time: '2018-12-23', open: 45.12, high: 53.90, low: 45.12, close: 48.09 },
            { time: '2018-12-24', open: 60.71, high: 60.71, low: 53.39, close: 59.29 },
            { time: '2018-12-25', open: 68.26, high: 68.26, low: 59.04, close: 60.50 },
            { time: '2018-12-26', open: 67.71, high: 105.85, low: 66.67, close: 91.04 },
    ...
    return (
        <div className="root-graph-container">
            <div className="buttons">
                ...
                <button onClick={() => setGraphSeriesType('candlestick')}>Candlestick View</button>
            ...
            {indicatorsTransform.map(indicator => (
                <div key={indicator.name} ref={ref => (indicatorGraphsRefs.current[indicator.name] = ref)} className="indicator-container"></div>
            ))}
        </div>
    );
};