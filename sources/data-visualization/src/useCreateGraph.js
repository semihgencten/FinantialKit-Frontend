...
export const useCreateGraph = (mainChartRef, indicatorGraphsRefs, graphDataSeries, graphIndicatorTuples, graphSeriesType, graphLightMode) => {
    ...
    useEffect(() => {
        const initializeMainChart = () => {
            ...
            if (!mainChartRef.current.chart) {
                const chart = createChart(mainChartRef.current, getChartLayout(graphLightMode, "main", graphIndicatorTuples));
                mainChartRef.current.chart = chart;

                if (graphDataSeries && graphDataSeries.length > 0) {
                    initializeSeriesForMainChart(chart, graphDataSeries, graphSeriesType, graphLightMode);
                }
                chart.timeScale().fitContent();
            } else {
                mainChartRef.current.chart.applyOptions(getChartLayout(graphLightMode, "main", graphIndicatorTuples));
            }
        };
        ...
        const initializeIndicatorCharts = () => {
            Object.keys(indicatorGraphsRefs.current).forEach(indicatorName => {
                ...
                if (!indicatorGraphRef.chart) {
                    const chart = createChart(indicatorGraphRef, {...getChartLayout(graphLightMode, "indicator", graphIndicatorTuples), height: 200});
                    indicatorGraphRef.chart = chart;

                    if (indicatorName === "volume") {
                        initializeSeriesForIndicator(chart, indicatorData.data, indicatorData.name, graphLightMode);
                    }
                    chart.timeScale().fitContent();
                } else {
                    indicatorGraphRef.chart.applyOptions({...getChartLayout(graphLightMode, "indicator", graphIndicatorTuples), height: 200});
                }
    ...
    }, [mainChartRef, indicatorGraphsRefs, graphDataSeries, graphIndicatorTuples, graphSeriesType, graphLightMode]);
};

function getChartLayout(graphLightMode, graphTypeMainOrIndicator, graphIndicatorTuples) {
    ...
    const timeScaleVisible = !(graphTypeMainOrIndicator === "main" && graphIndicatorTuples.length > 0);
    ...
    return {
        layout: {
            ...
            textColor: graphLightMode === 'dark' ? 'rgba(255, 255, 255, 0.9)' : 'rgba(0, 0, 0, 0.9)',
        },
        grid: {
            vertLines: {
                color: graphLightMode === 'dark' ? 'rgba(255, 255, 255, 0.5)' : 'rgba(0, 0, 0, 0.2)',
                style: 1,
                width: 0.5,
            },
            horzLines: {
