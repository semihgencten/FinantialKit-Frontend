...
export const Graph = ({ graphLightMode = "light", selectedIndicatorsList = [], viewMode = "simple" }) => {
    const chartRef = useRef(null);
    const data = useChartData('https://demo-live-data.highcharts.com/aapl-ohlcv.json');
    ...
    const updateGraphForLightOrDarkMode = () => {
        chartRef.current.update({
            ...
            tooltip: {
                ...
                backgroundColor: graphLightMode === "light" ? 'rgba(255, 255, 255, 0.65)' : 'rgba(0, 0, 0, 0.5)', 
    ...
    const addIndicatorSeries = (chart, axisName, indicator, data) => {
        ...
        }, false); // prevent immediate redraw
    }
    ...
    const updateGraphIndicators = () => {
        ...
        const axesToRemove = chart.yAxis.filter(axis => {
            return axis.userOptions.id !== 'mainAxis' && axis.userOptions.id !== 'navigator-y-axis' && !selectedIndicatorsList.some(indicator => axis.userOptions.id === indicator.technicalName + 'Axis');
        });
        ...
        chart.yAxis.forEach((axis) => {
            ...
            if (indicator) { // If the axis is found in the selectedIndicatorsList
                ...
                }, false); // false to delay the redraw
            }
            }
        });
    }
