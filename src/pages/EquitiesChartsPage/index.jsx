...
import { Box, Button, Dialog, Grid, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow } from "@mui/material";
...
const EquitiesChartsPage = () => {
    ...
    const [selectedIndicatorsListTemporary, setSelectedIndicatorsListTemporary] = useState([]);
    ...
    const [indicators, setIndicators] = useState([
        ...
        { technicalName: "rsi", displayName: "Stochastic Indicator", location: "side" },
        { technicalName: "mom", displayName: "Momentum", location: "side" },
        { technicalName: "bop", displayName: "Balance of Power", location: "side" },
        { technicalName: "ma", displayName: "Moving Average", location: "main", options: [{val: "ma7", sel: false}, {val: "ma14", sel: false}, {val: "ma30", sel: false}, {val: "ma60", sel: false}, {val: "ma120", sel: false}] },
        { technicalName: "xyz", displayName: "XYZ", location: "side" },
        { technicalName: "boll", displayName: "Bollinger Bands", location: "main", options: [{val: 1, sel: false}, {val: "boll5", sel: false}, {val: "boll7", sel: false}, {val: "boll10", sel: false}, {val: "boll12", sel: false}] },
    ]);

    const handleIndicatorClick = (indicator) => {
        if (indicator.location === "side") {
            // Check if the indicator already exists in the selectedIndicatorsListTemporary
            const existingIndicator = selectedIndicatorsListTemporary.find(item => item.technicalName === indicator.technicalName);
            if (!existingIndicator) {
                // If not, add the indicator to the selectedIndicatorsListTemporary
                setSelectedIndicatorsListTemporary([...selectedIndicatorsListTemporary, indicator]);
            } else {
                // If exists, remove it from the temporary list
                const updatedIndicatorsList = selectedIndicatorsListTemporary.filter(item => item.technicalName !== indicator.technicalName);
    ...
    const handleOptionSelection = (option) => {
        ...
        const updatedIndicatorsList = isOptionSelected
            ? selectedIndicatorsListTemporary.filter((indicator) => indicator.technicalName !== indicatorToRemove)
    ...
    const handleSubmitIndicators = () => {
        ...
        const uniqueIndicatorsSet = new Set(selectedIndicatorsListTemporary.map((indicator) => JSON.stringify(indicator)));
        // Convert the Set back to an array of unique indicators
        const uniqueIndicatorsArray = Array.from(uniqueIndicatorsSet).map((indicator) => JSON.parse(indicator));
        setSelectedIndicatorsList(uniqueIndicatorsArray);
        
        console.log(uniqueIndicatorsArray);
        if (uniqueIndicatorsArray.filter((indicator) => indicator.location === "side").length > 0) {
            setViewMode("complex");
        } else {
            setViewMode("simple");
        }
    ...
    const handleResetIndicators = () => {
        ...
        const updatedIndicators = indicators.map(indicator => {
            if (indicator.options) {
                const updatedOptions = indicator.options.map(option => ({ ...option, sel: false }));
                        return { ...indicator, options: updatedOptions };
            }
            return indicator;
        });
    ...
    return (
        <TabsLayout>
            <Box sx={{ display: "flex", flexDirection: "column", textAlign: "center", gap: "8px" }}>
                <h2>Charts Page</h2>
                <Button variant="outlined" onClick={() => setShowIndicatorsDialog(true)} sx={{ maxWidth: "320px"}}>Add/Remove Indicators</Button>
                <Dialog open={showIndicatorsDialog} onClose={() => setShowIndicatorsDialog(false)} PaperProps={{ sx: { maxWidth: "revert", width: "70vw", maxHeight: "50vh", overflowY: "auto" } }} >
                    <Grid container>
                        {/* Left column */}
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {indicators.map((indicator, index) => (
                                                <TableRow key={index} onClick={() => handleIndicatorClick(indicator)}>
                                                    <TableCell>
                                                        {indicator.displayName}
                                                        {selectedIndicatorsListTemporary.some(item => item.technicalName === indicator.technicalName) && (
                                                            <CheckCircleIcon color="primary" style={{ verticalAlign: "middle", float: "right" }} />
                                                        )}
                                                        {indicator.options && (
                                                            <span style={{ verticalAlign: "middle", float: "right", marginRight: "8px" }}>
                            ...
                            <Paper>
                                <TableContainer>
                                    <Table>
                                        ...
                                        <TableBody>
                                            {selectedOption && indicators.find(indicator => indicator.technicalName === selectedOption.technicalName)?.options.map((option, index) => (
                                                <TableRow key={index} onClick={() => handleOptionSelection(option)}>
                                                    <TableCell>
                                                        {option.val}
                                                        {option.sel && (
                                                            <CheckCircleIcon color="primary" style={{ verticalAlign: "middle", float: "right" }} />
                        ...
                        <Box sx={{ display: "flex", flexDirection: "row", placeContent: "center", width: "100%", paddingY: "16px", gap: "16px" }}>
                            <Button variant="contained" onClick={handleSubmitIndicators} sx={{ height: "30px"}}>Submit</Button>
                            <Button variant="contained" onClick={handleResetIndicators} sx={{ height: "30px"}}>Reset</Button>
                        </Box>
                    </Grid>
                </Dialog>
